"""Re-write mmdeploy/codebase/mmdet/models/detectors/single_stage.py for otx."""
...
@mark("detector_forward", inputs=["input"], outputs=["dets", "labels", "feats", "saliencies"])
def __forward_impl(ctx, self, batch_inputs, data_samples):
    ...
    output = self.bbox_head.predict_by_feat(*outs, batch_img_metas=batch_img_metas, rescale=False)
    ...
    if ctx.cfg["dump_features"]:
        ...
        return (*output, feature_vector, saliency_map)
    return output
...
@FUNCTION_REWRITER.register_rewriter("mmdet.models.detectors.single_stage.SingleStageDetector.forward")
def single_stage_detector__forward(
    ...
    Args:
        self (SingleStageDetector): SingleStageDetector class.
    ...
    return __forward_impl(ctx, self, batch_inputs, data_samples=data_samples)
